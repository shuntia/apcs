...
public class Checkers extends JPanel {
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
        CheckersData data; // The data for the checkers board is kept here.
        ...
        void doNewGame() {
            ...
            data.setUpGame();   // Set up the pieces.
            currentPlayer = CheckersData.RED;   // RED moves first.
            legalMoves = data.getLegalMoves(CheckersData.RED);  // Get RED's legal moves.
        ...
        void doMakeMove(CheckersMove move) {
            ...
            if (move.isJump()) {
                legalMoves = data.getLegalJumpsFrom(currentPlayer,move.toRow,move.toCol);
                if (legalMoves != null) {
                    if (currentPlayer == CheckersData.RED)
            ...
            if (currentPlayer == CheckersData.RED) {
                ...
                legalMoves = data.getLegalMoves(currentPlayer);
                if (legalMoves == null)
        ...
        public void paintComponent(Graphics g) {
            ...
            for (int row = 0; row < data.board.length; row++) {
                for (int col = 0; col < data.board[0].length; col++) {
                    if ( row % 2 == col % 2 )
                        g.setColor(Color.LIGHT_GRAY);
                    else
                        g.setColor(Color.GRAY);
                    g.fillRect(2 + col*20, 2 + row*20, 20, 20);
                    switch (data.pieceAt(row,col)) {
